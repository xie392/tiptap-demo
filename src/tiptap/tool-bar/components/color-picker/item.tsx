...
export const ColorPickerItem: React.FC<ColorPickerItemProps> = ({
  ...
  return (
    <span
      className={`w-6 h-6 p-0.5 inline-block rounded-[3px] border border-transparent cursor-pointer bg-transparent hover:border hover:bg-white hover:border-zinc-200 hover:shadow
        ${isTransparent && "relative"}
        ${
          isTransparent &&
          "after:absolute after:top-[10px] after:left-0 after:w-[22px] after:h-0 after:border-b-2 after:border-[#ff5151] after:border-solid after:transform after:rotate-45deg"
        }
        `}
    ...
    >
      <span
        className={`relative w-[18px] h-[18px] block rounded-sm border border-transparent
          ${needBorder && `border border-zinc-200`}
          `}
        style={styles.block}
      >
        <svg
