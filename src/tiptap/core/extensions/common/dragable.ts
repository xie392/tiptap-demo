...
import { ActiveNode, removePossibleTable, selectAncestorNodeByDom, safePos } from "tiptap/shared"
...
const Draggable = Extension.create({
  ...
  addProseMirrorPlugins() {
    ...
    const renderDragHandleDOM = (view: EditorView, referenceRectDOM: HTMLElement) => {
      ...
      dragHandleDOM.style.top = `${top - 1}px`
    ...
    const handleDragStart = (event: any) => {
      ...
      if (event.dataTransfer && activeSelection) {
        ...
        const { dom, text } = slice.content.toJSON()
      ...
      new PMPlugin({
        ...
        view: (view) => {
          if (view.editable) {
            ...
            if (view.dom.parentNode instanceof HTMLElement) {
              // view.dom.parentNode.style = "position: relative;"
              view.dom.parentNode.setAttribute("style", "position: relative;")
            }
          }
        ...
        props: {
          ...
          handleDOMEvents: {
                ...
                if (activeSelection) {
                  ;[
                    ...
                    ;(view.dom as HTMLElement)
                      ...
                      .forEach((dom) => dom.classList.remove(cls))
                  })
            ...
            mousemove: (view, event) => {
              ...
              const maybeTaskItemOrListItem = findParentNodeClosestToPos(
                view.state.doc.resolve(pos.pos),
                (node) => ["taskItem", "listItem"].some((name) => name === node.type.name)
              )
              ...
              if (maybeTaskItemOrListItem) {
                while (dom && (dom as HTMLElement).tagName !== "LI") {
                  dom = dom.parentElement as HTMLElement
                }
              }

              if ((dom as HTMLElement).tagName === "LI") {
              ...
              // 不允许选中整个列表
              if ((dom as HTMLElement).tagName === "UL" || (dom as HTMLElement).tagName === "OL") {
                return false
              }

              try {
                let maybeReactRenderer: HTMLElement | null = dom as HTMLElement

                while (
                  maybeReactRenderer &&
                  !maybeReactRenderer.classList?.contains("react-renderer")
                ) {
                ...
                if (
                  maybeReactRenderer &&
                  !maybeReactRenderer?.classList?.contains("node-columns")
                ) {
